...
class ModelManager:
    ...
    def init_model(self, model_type, u_size, v_size, t_size,candidate,vid_coor_nor):
        if model_type == 'birnn':
            return BiRNN(v_size, self.opt['emb_dim_v'], self.opt['hidden_dim'])
        elif model_type == 'birnnt':
            return BiRNNT(v_size, t_size, u_size, self.opt['emb_dim_v'], self.opt['emb_dim_t'],self.opt['emb_dim_u'],self.opt['emb_dim_d'],self.opt['hidden_dim'],self.opt['nb_cnt'],candidate,vid_coor_nor)
    ...
    def build_model(self, model_type, dataset):
        print 'build_model'
        model = self.init_model(model_type, dataset.u_vocab.size(), dataset.v_vocab.size(), dataset.t_vocab.size(),dataset.candidate,dataset.vid_coor_nor)

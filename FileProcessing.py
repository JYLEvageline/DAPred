...
def distance_data(root_path):
    ...
    distance = pickle.dump(candidate, open(root_path + "candidate.pk", 'wb'))
    f_distance = open(root_path + "distance.txt", 'w')
    vid_coor_nor = np.loadtxt(coor_file, delimiter=',', dtype=np.float64)
    for i in range(len(vid_coor_nor)):
        distance = []
        for j in range(len(vid_coor_nor)):
    ...
    for i in range(len(vid_coor_nor)):
        for j in range(len(vid_coor_nor)):
            distance[i][j] = np.sqrt(np.sum((vid_coor_nor[i] - vid_coor_nor[j]) ** 2))
    pickle.dump(distance, open(root_path + "distance.pk", 'wb'))
...
def candidate(root_path):
    dl = pickle.load(open(root_path + 'dl.pk', 'rb'))
    vid_coor_nor = dl.vid_coor_nor
    vid_pop = dl.vid_pop
    candidate = {}
    tree = KDTree(vid_coor_nor.values())
    for i in range(len(vid_coor_nor)):
        print(i)
        nbs = set()
        vid_dist = {}
        dists, vids = tree.query([vid_coor_nor[i]], 50)
        for j,vid in enumerate(vids[0]):
            vid = vids[0, j]
            nbs.add(vid)
            if vid not in vid_dist or dists[0, 1] < vid_dist[vid]:
                vid_dist[vid] = dists[0, j]
        vids = list(nbs)
        probs_pop = np.array([vid_pop[vid] for vid in vids], dtype=np.float64)
        probs_dist = np.array([np.exp(-vid_dist[vid]) for vid in vids], dtype=np.float64)
        probs_pop /= probs_pop.sum()
        probs_dist /= probs_dist.sum()
        candidate[i] = (vids, probs_pop, probs_dist)

    pickle.dump(candidate, open(root_path+"candidate.pk", 'wb'))
    return vid_coor_nor
...
if __name__ == "__main__":
    ...
    # get the candidate
    #vid = candidate('LocationCuda/'+'small/foursquare/')

    #a = pickle.load(open('LocationCuda/'+'small/foursquare/' + 'candidate.pk', 'rb'))

...
class BiRNN(nn.Module):
    ...
    def get_hiddens_long(self, vids_long, len_long, mask_long_valid):
        ...
        hidden_long = self.init_hidden(vids_long.size(0)) #number of record not length of each record not
        ...
        _, idx_original = idx_sorted.sort(0, descending=False) # return to unsorted
        index_original_idx = idx_original.view(-1, 1, 1).expand_as(unpack_hiddens)
        output = unpack_hiddens.gather(0, index_original_idx.long()) #output contains all
        output_linear = output.view(-1, self.hidden_dim) #output_linear contains first( what's the meaning of "first")
